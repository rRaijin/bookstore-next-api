...
export const up = async (db, client) => {
    ...
    const getGenresIds = (nums) => {
        ...
        nums.forEach((num) => {
            ...
            switch (num) {
                case 1:
                    gId = genres.find((g) => g.title === 'Роман')._id;
                    break;
                case 2:
                    gId = genres.find((g) => g.title === 'Детская литература')._id;
                    break;
                case 3:
                    gId = genres.find((g) => g.title === 'Сатира')._id;
                    break;
                case 4:
                    gId = genres.find((g) => g.title === 'Детектив')._id;
                    break;
                case 5:
                    gId = genres.find((g) => g.title === 'Приключения')._id;
                    break;
                case 6:
                    gId = genres.find((g) => g.title === 'Научная фантастика')._id;
                    break;
                case 7:
                    gId = genres.find((g) => g.title === 'Новелла')._id;
                    break;
            }
    ...
    const items = [
        {
            ...
            description:
                '«Приключения Тома Сойера» — вышедшая в 1876 году повесть Марка Твена о приключениях мальчика, живущего в небольшом американском городке Сент-Питерсберг в штате Миссури.',
            year: 1876,
            price: 500,
        ...
        },
        {
            ...
            description:
                'Здесь будут жених, опасающийся мести бывшей возлюбленной, и невеста, брошенная в день венчания; загадочные апельсиновые зернышки и тайный код пляшущих человечков, смертоносный китобойный гарпун и рождественский гусь с сюрпризом…',
            year: 1876,
            price: 800,
        ...
        },
        {
            ...
            description:
                'Профессор Аронакс и его друзья волею судьбы оказываются на борту подводного судна «Наутилус», которым управляет загадочный капитан Немо.',
            year: 1863,
            price: 1200,
            authorId: new ObjectId(getAuthorId('juliewern@gmail.com')),
